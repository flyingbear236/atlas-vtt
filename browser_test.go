...
func TestBrowser(t *testing.T) {
 ...
 await wait(()=>document.querySelectorAll('.tree-element-row').length===2);const rotationRow=document.querySelector('.tree-element-row');rotationRow.click();const allowRotation=document.getElementById('allowElementRotation');if(document.getElementById('fixElementRotation'))throw Error('fix rotation UI still exposed');if(allowRotation.hidden)throw Error('base rotation opt-in missing');allowRotation.click();if(!allowRotation.hidden)throw Error('base rotation opt-in not applied');document.getElementById('elementRotation').value='37';document.getElementById('elementProperties').requestSubmit();await wait(()=>Number(document.getElementById('elementRotation').value)===37);if(sentCommands.some(command=>command.type==='elementFixRotation'))throw Error('disabled fix rotation command was sent');
